...
for rep_num in xrange(pa.reps):
	...
	### agent does stuff here        
	rqs.update_state(a,reward,terminal,o)
	if pa.train:
	...
	# if we're training, add to replay memory
	if pa.train:
		rqs.remember(o,a,reward,terminal)
		# train every train_interval steps, only after train_start
		if rep_num>train_start && rep_num%train_interval==train_interval-1:
