...
class Agent(BaseModel):
    ...
    lam=0.5 # parameter in [0,1], tradeoff b/n model-free and model-based
    ...
    def update_lstm_states(self, act_prev, rew_prev, screen, terminal):
        if not terminal:
            self.lstm_out_prev, self.lstm_state = self.sess.run([self.e_lstm_out, self.lstm_state_end],
                                                                {self.lstm_state_start: self.lstm_state,
                                                                 self.e_s_t: np.expand_dims(np.expand_dims(screen, 0),
                                                                                            0)})
        else:
            self.reset_lstm_states()
    ...
    def embed_a(self, a):
        a_in = tf.placeholder("uint8", shape=a.shape)
        return self.sess.eval(tf.one_hot(a_in, self.env.action_size), {a_in : a})
    ################################
    ...
    def build_dqn(self):
        ...
        with tf.variable_scope('prediction'):
            ...
            action_0_one_hot = tf.squeeze(
                tf.one_hot(self.action_0, self.env.action_size, 1.0, 0.0, name='action_0_one_hot'), [2])
            ...
            self.lstm_out, _, self.w['lstm_w'], self.w['lstm_b'] = lstm(self.lstm_in, self.init_state,
                                                                        self.lstm_out_size, name='lstm')

            self.l4, self.w['l4_w'], self.w['l4_b'] = linear_time_dist(self.lstm_out, 64, activation_fn=activation_fn,
                                                                       name='l4')
            ...
            k1_input = tf.concat(2, [self.lstm_out, action_0_one_hot])
            self.k1, self.w['k1_w'], self.w['k1_b'] = linear_time_dist(k1_input, self.lstm_out_size + self.env.action_size/2, activation_fn=activation_fn,
                                                                       name='k1')
            self.p, self.w['p_w'], self.w['p_b'] = linear_time_dist(self.k1, self.lstm_out_size + 1, activation_fn=activation_fn, name='p') # plus 1 for reward
        ...
        with tf.variable_scope('eval'):
            ...
            e_action_0_one_hot = tf.squeeze(
                tf.one_hot(self.e_action_0, self.env.action_size, 1.0, 0.0, name='e_action_0_one_hot'), [2])
            ...
            self.e_lstm_out, self.lstm_state_end, self.e_w['lstm_w'], self.e_w['lstm_b'] = lstm(self.e_lstm_in,
                                                                                                self.lstm_state_start,
                                                                                                self.lstm_out_size,
                                                                                                name='e_lstm')  # here we use self.lstm_state

            self.e_l4, self.e_w['l4_w'], self.e_w['l4_b'] = linear_time_dist(self.e_lstm_out, 64,
                                                                             activation_fn=activation_fn, name='e_l4')
            ...
            e_k1_input = tf.concat(2, [self.e_lstm_out, e_action_0_one_hot])
            self.e_k1, self.e_w['k1_w'], self.e_w['k1_b'] = linear_time_dist(e_k1_input,
                                                                       self.lstm_out_size + self.env.action_size / 2,
                                                                       activation_fn=activation_fn,
                                                                       name='e_k1')
            self.e_p, self.e_w['p_w'], self.e_w['p_b'] = linear_time_dist(self.e_k1, self.lstm_out_size + 1, activation_fn=activation_fn, name='e_p')
        ...
        with tf.variable_scope('target'):
            ...
            target_action_0_one_hot = tf.squeeze(
                tf.one_hot(self.target_action_0, self.env.action_size, 1.0, 0.0, name='target_action_0_one_hot'), [2])
            ...
            self.target_lstm_out, _, self.t_w['lstm_w'], self.t_w['lstm_b'] = lstm(self.target_lstm_in,
                                                                                   target_init_state,
                                                                                   self.lstm_out_size,
                                                                                   name='target_lstm')
